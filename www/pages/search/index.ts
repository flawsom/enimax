(async function () {
    ...
    // @ts-ignore
    const extensionNames : Array<string> = await postMessagePromise(window.parent, {
        "action": "returnExNames"
    });

    // @ts-ignore
    const extensionDisabled = await postMessagePromise(window.parent, {
        "action": "returnExDis"
    });
    ...
    function search() {
        ...
        postMessagePromise(window.parent, {
            "action": "searchExtension",
            "engine" : currentEngine,
            "value" : searchInput.value
        }).then(function (x : extensionSearch) {
